...
import BrandData from '../../config copy/data/car_manufacturers.json'
import YearData from '../../config copy/data/years.json'
...
const CreateListingPage = ({ createListing, history, isAuthenticated }) => {
  ...
  CarYear = CarYear.sort((a, b) => a - b);

  const MakeItem = function (X) {
    return <option value={X}>{X}</option>;
  };
  const Brand = CarBrand.map(MakeItem)
  const Year = CarYear.map(MakeItem)
    ...
    <Fragment>
      ...
      <Form style={{ width: '100%' }} className="mt-5" encType='multipart/form-data' onSubmit={e => onSubmit(e)}>
        ...
        <FormGroup  >
          ...
          <select className='form-control form-control' value={make} onChange={e => setMake(e.target.value)} name='make' id="make" required>
        ...
        <FormGroup >
          ...
          <select className='form-control form-control' value={year} onChange={e => setYear(e.target.value)} name='year' id="year" required>
            <option value="" selected></option>
            {Year}
          </select>
        </FormGroup>
